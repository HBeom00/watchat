    ...
    <>
      {searchText === '' ? (
        <div className="embla w-full, h-full relative mt-[16px] overflow-hidden" ref={emblaRef}>
          ...
          <button
            className="absolute left-[220px] top-1/2 transform -translate-y-1/2 z-10"
            onClick={scrollPrev}
          >
           <Image
            src="/arrow_left_2.svg" 
            alt="User Icon"
            width={24}
            height={24}
            />
          ...
          <button
            className="absolute right-[220px] top-1/2 transform -translate-y-1/2 z-10"
            onClick={scrollNext}
          >
            <Image
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
            {[...Array(slidesInView)].map((_, index) => (
              <button
                key={index}
                className={` rounded-full ${index === selectedIndex ? 'bg-primary-500 w-[24px] h-[4px]' : 'bg-primary-200 w-[4px] h-[4px]'}`}
